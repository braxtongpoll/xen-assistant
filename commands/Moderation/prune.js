...
exports.run = async(client, message, args) => {
    client.db.findById(client.user.id, async function(err, res) {
        ...
        if (message.mentions.members.first() && args[1] < 101) {
            ...
            await fs.writeFileSync(path.join(__dirname, "../", "../", "/src", "/local", "/prune.txt"), logger);
            ...
            if (modlogs) {
                var embed = new MessageEmbed()
                    ...
                    .attachFiles(path.join(__dirname, "../", "../", "/src", "/local", "/prune.txt"))
                modlogs.send(embed);
            }
        } else if (args[0] > 100) {
            if (!args[1]) return client.utils.missing(message, `NUMBER_OF_MESSAGES`);
            if (isNaN(args[1])) return client.utils.missing(message, `NUMBER_OF_MESSAGES`);
            const member = message.mentions.members.first() || await message.guild.members.cache.find(c => c.id === args[0]);
            if (!member) return client.missing(message, exports.info.arguments)
            message.delete().catch(e => {});
            message.channel.messages.fetch({ limit: args[1] }).then(collected => {
                collected.forEach(msg => {
                    if (member.id == args[0]) msg.delete().catch(e => {});
                    messages[msg.id] = {
                        author: { tag: msg.author.tag, id: msg.author.id },
                        content: msg.content || `Unknown`
                    }
